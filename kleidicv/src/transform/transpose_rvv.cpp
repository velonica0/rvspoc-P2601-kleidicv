// SPDX-FileCopyrightText: 2026 RVSPOC Contributors
//
// SPDX-License-Identifier: Apache-2.0

#include "kleidicv/rvv.h"
#include "kleidicv/transform/transpose.h"

namespace kleidicv::neon {

// Scalar transpose implementation for all element sizes.
template <typename T, bool k3Channels = false>
static kleidicv_error_t transpose(const void *src_void, size_t src_stride,
                                  void *dst_void, size_t dst_stride,
                                  size_t src_width, size_t src_height) {
  MAKE_POINTER_CHECK_ALIGNMENT(const T, src, src_void);
  MAKE_POINTER_CHECK_ALIGNMENT(T, dst, dst_void);
  CHECK_POINTER_AND_STRIDE(src, src_stride, src_height);
  CHECK_POINTER_AND_STRIDE(dst, dst_stride, src_width);
  CHECK_IMAGE_SIZE(src_width, src_height);

  constexpr size_t ch = k3Channels ? 3 : 1;

  if (src == dst) {
    if (src_width != src_height) {
      return KLEIDICV_ERROR_NOT_IMPLEMENTED;
    }
    // In-place transpose (square only) -- kept scalar
    for (size_t i = 0; i < src_height; ++i) {
      for (size_t j = i + 1; j < src_width; ++j) {
        T *a = row_ptr(dst, dst_stride, i) + j * ch;
        T *b = row_ptr(dst, dst_stride, j) + i * ch;
        for (size_t c = 0; c < ch; ++c) {
          std::swap(a[c], b[c]);
        }
      }
    }
  } else {
#ifdef __riscv_vector
    // RVV vectorised out-of-place transpose for single-channel data.
    // Uses contiguous load from src row + strided store to dst column.
    if constexpr (!k3Channels && sizeof(T) <= 4) {
      for (size_t i = 0; i < src_height; ++i) {
        const T *src_row = row_ptr(src, src_stride, i);
        size_t j = 0;
        while (j < src_width) {
          size_t vl = rvv_setvl<T>(src_width - j);
          // Destination: column i, rows j..j+vl-1.
          // Base address = &dst[j][i], stride between consecutive rows =
          // dst_stride bytes.
          T *dst_col = row_ptr(dst, dst_stride, j) + i;
          if constexpr (sizeof(T) == 1) {
            vuint8m1_t v = __riscv_vle8_v_u8m1(
                reinterpret_cast<const uint8_t *>(src_row + j), vl);
            __riscv_vsse8_v_u8m1(reinterpret_cast<uint8_t *>(dst_col),
                                 static_cast<ptrdiff_t>(dst_stride), v, vl);
          } else if constexpr (sizeof(T) == 2) {
            vuint16m1_t v = __riscv_vle16_v_u16m1(
                reinterpret_cast<const uint16_t *>(src_row + j), vl);
            __riscv_vsse16_v_u16m1(reinterpret_cast<uint16_t *>(dst_col),
                                   static_cast<ptrdiff_t>(dst_stride), v, vl);
          } else {  // sizeof(T) == 4
            vuint32m1_t v = __riscv_vle32_v_u32m1(
                reinterpret_cast<const uint32_t *>(src_row + j), vl);
            __riscv_vsse32_v_u32m1(reinterpret_cast<uint32_t *>(dst_col),
                                   static_cast<ptrdiff_t>(dst_stride), v, vl);
          }
          j += vl;
        }
      }
    } else
#endif  // __riscv_vector
    {
      // Scalar fallback for 3-channel, u64, or non-RVV builds.
      // Out-of-place transpose: dst[j][i] = src[i][j]
      for (size_t i = 0; i < src_height; ++i) {
        const T *src_row = row_ptr(src, src_stride, i);
        for (size_t j = 0; j < src_width; ++j) {
          T *dst_pixel = row_ptr(dst, dst_stride, j) + i * ch;
          const T *src_pixel = src_row + j * ch;
          for (size_t c = 0; c < ch; ++c) {
            dst_pixel[c] = src_pixel[c];
          }
        }
      }
    }
  }
  return KLEIDICV_OK;
}

KLEIDICV_TARGET_FN_ATTRS
kleidicv_error_t transpose(const void *src, size_t src_stride, void *dst,
                           size_t dst_stride, size_t src_width,
                           size_t src_height, size_t pixel_size) {
  switch (pixel_size) {
    case sizeof(uint8_t):
      return transpose<uint8_t>(src, src_stride, dst, dst_stride, src_width,
                                src_height);
    case sizeof(uint16_t):
      return transpose<uint16_t>(src, src_stride, dst, dst_stride, src_width,
                                 src_height);
    case sizeof(uint32_t):
      return transpose<uint32_t>(src, src_stride, dst, dst_stride, src_width,
                                 src_height);
    case sizeof(uint64_t):
      return transpose<uint64_t>(src, src_stride, dst, dst_stride, src_width,
                                 src_height);
    case sizeof(uint8_t) * 3:
      return transpose<uint8_t, true>(src, src_stride, dst, dst_stride,
                                      src_width, src_height);
    case sizeof(uint16_t) * 3:
      return transpose<uint16_t, true>(src, src_stride, dst, dst_stride,
                                       src_width, src_height);
    default:
      return KLEIDICV_ERROR_NOT_IMPLEMENTED;
  }
}

}  // namespace kleidicv::neon
